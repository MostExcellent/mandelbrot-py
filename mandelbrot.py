from PIL import Image
from helpers import *
from math import log, sqrt
from datetime import datetime
from argparse import ArgumentParser

def mandelbrot(myX, myY, zoom, width, height):
	maxiter = zoom*50 # number of iterations before escape
	xFocus = (myX + (1.75/zoom))
	yFocus = -(myY + (-1/zoom))
	pixels = []
	for i in range(0, height):
		for n in range(0, width):
			#
			cX = mapRange(n, 0, width, -3.5/float(zoom), 0) + xFocus
			cY = mapRange(i, 0, height, -2/float(zoom), 0) + yFocus
			x = 0
			y = 0
			iteration = 0
			while x*x + y*y < 2*2 and iteration < maxiter:
				# while inside bailout radius of 2 and number of iterations has not reached the maximum
				xtemp = x*x - y*y + cX
				y = 2*x*y + cY
				x = xtemp
				iteration += 1
			if iteration == maxiter:
				pixels.append((0,0,0))
			else:
				mu = float(iteration) - log(log(sqrt(x*x+y*y))) / log(2)+1 #normalized iteration count
				iCpct = float(mu/maxiter)
				iC = iCpct*254
				c1 = int(iC)
				c2 = c1 + 1
				t = int(mapRange(linterp(c1,c2,(float)(mu % 1)),0,256,1,255))
				pixels.append(colorMap(t))
	return imgFromTuples('RGB',(width,height),pixels)

if __name__ == '__main__':
	ts = datetime.now()
	parser = ArgumentParser()
	parser.add_argument("x", type=float, help="x coordinate of the center of the plot", default = -0.75)
	parser.add_argument("y", type=float, help="y coordinate of the center of the plot", default = 0.0)
	parser.add_argument("zoom", type=int, help="Zoom level of the plot", default = 1)
	parser.add_argument("w", type=int, help="Width of the plot", default = 480)
	parser.add_argument("h", type=int, help="Height of the plot", default = 720)
	parser.add_argument("filename", type=str, help="Specify a filename for the output")
	args = parser.parse_args()
	print str(args.x) + ", " + str(args.y) + ", " + str(args.zoom) + ", " + str(args.w) + ", " + str(args.h) + ", " + args.filename + ".tiff"
	plot = mandelbrot(args.x, args.y, args.zoom, args.w, args.h)
	plot.save(args.filename + ".tiff")
	print "Done!"
	print "Finished in %i milliseconds" % int((datetime.now()-ts).total_seconds() * 1000)